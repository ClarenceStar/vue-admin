    ...
    <el-row class="container">
            ...
            <el-col :span="4" class="userinfo">
                <el-dropdown trigger="hover">
                    ...
                    <el-dropdown-menu slot="dropdown">
                        <el-dropdown-item>
                            <i class="el-icon-message" /> 我的消息</el-dropdown-item>
        ...
        <el-col :span="24" class="main">
            <aside :class="collapsed?'menu-collapsed':'menu-expanded'">
                <!--导航菜单-->
                <el-menu :background-color="bgColor" text-color="#fff" active-text-color="#ffd04b" v-bind:style=" collapsed ? {} : { 'width': '230px' } " :default-active="$route.path" class="el-menu-vertical-demo" unique-opened router v-show="!collapsed">
                    <template v-for="(item,index) in $router.options.routes" v-if="!item.hidden">
                        ...
                        <el-menu-item v-if="item.leaf&&item.children.length>0" :index="item.children[0].path" :key="item.id">
                            <i :class="item.iconCls"></i>{{item.children[0].name}}
                        </el-menu-item>
                    </template>
                    <div class="switch">
                        <el-switch v-model="themeSwitch" :change="changeTheme">
                        </el-switch>
                    </div>
                </el-menu>
                <!--导航菜单-折叠后-->
                <ul class="el-menu el-menu-vertical-demo collapsed el-menu-collapsed" v-show="collapsed" ref="menuCollapsed">
                    <li v-for="(item,index) in $router.options.routes" v-if="!item.hidden" class="el-submenu item" :key="item.id">
                        <template v-if="!item.leaf">
                            ...
                            <ul class="el-menu submenu" :class="'submenu-hook-'+index" @mouseover="showMenu(index,true)" @mouseout="showMenu(index,false)">
                                <template v-if="!item.multi">
                                    <li v-for="child in item.children" :key="child.path" class="el-menu-item" style="padding-left: 40px; color: #fff;" :class="$route.path==child.path?'is-active':''" @click="$router.push(child.path)">
                                        <i :class="child.iconCls"></i>{{child.name}}
                                    </li>
                                </template>
                                <template v-else>
                                    <li v-for="(child,i) in item.children" :key="child.path" class="el-menu-item" style="padding-left: 40px; color: #fff;" :class="$route.path==child.path?'is-active':''">
                                        ...
                                        <template v-else>
                                            ...
                                            <ul class="submenu" :class="'submenu-hook-'+i" @mouseover="showMenu(i,true)" @mouseout="showMenu(i,false)">
                                                <li v-for="subItem in child.children" v-if="!subItem.hidden" :key="subItem.path" class="el-menu-item" style="margin-left:100px" :class="$route.path==subItem.path?'is-active':''" @click="$router.push(subItem.path)">
                                                    <i :class="subItem.iconCls"></i>{{subItem.name}}
                                                </li>
                                            </ul>
                                        </template>
